 ...
 * - calendar time (see \ref hal_os_caltime)
 * 
*/
/*@{*/
...
/** \defgroup hal_os_caltime OSCalTime interface
 *
 *  An interface to system calendar time provided by the OS.
 *  The implementation and the capabilities of the OSCalTime depend on the OS used.
 */
/*@{*/
 ...
 *
 * \param time calendar time to set
 * \return HALRESULT_OK on success. Other values indicate error.
 */
...
int64_t OSCALTIME_ToUS(OSCalTime);

OSCalTime OSCALTIME_FromUS(int64_t us);

int64_t OSCALTIME_ToNS(OSCalTime);
