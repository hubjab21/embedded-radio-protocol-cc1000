...
/** \defgroup hal_iobuf IOBuf module
 * 
 * 
 * <b>Introduction.</b>
 * 
 ...
 * 
 * Creating and destroying dynamic IOBufs requires dynamic memory manager. You can select one in
 * your config.h in the HEAP module section.
 * 
 */
/*@{*/

#if defined HAL_IOBUF_LIMIT_SIZE
    #if HAL_IOBUF_LIMIT_SIZE <= 8
		typedef uint_fast8_t iobuf_size_t;
	#elif HAL_IOBUF_LIMIT_SIZE <= 16
		typedef uint_fast16_t iobuf_size_t;
	#else
		typedef uint_fast32_t iobuf_size_t;
	#endif
#else
...
/**
 *  Reads a single element from a buffer, without utilizing any critical sections (fast call).
 *  This function is only safe to call when global interrupts are disabled, in example inside 
 *  non-nestable interrupt service routine.
 *  \param iobuf handle of the buffer
 *  \param elem destination buffer
 *  \return number of element read (0 or 1)
 */
iobuf_size_t IOBUF_ReadFast(IOBuf iobuf, void* elem);

/**
 *  Writes a single element to a buffer, without utilizing any critical sections (fast call).
 *  This function is only safe to call when global interrupts are disabled, in example inside 
 *  non-nestable interrupt service routine.
 *  \param iobuf handle of the buffer
 *  \param elem source data buffer
 *  \return number of element written (0 or 1)
 */
...
#define IOBUF_DECLARE(name, count, size)		     \
	static uint8_t name##_mempool[(count)*(size)]; \
	static IOBufDesc name = { 0, size, count, &name##_mempool, &name##_mempool, &name##_mempool };
