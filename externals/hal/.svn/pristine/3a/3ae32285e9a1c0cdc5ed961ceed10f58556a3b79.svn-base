...
--------------------------------------------------------------------------------
 ...
 Doc:           doxygen
--------------------------------------------------------------------------------
 Content:       Definitions of the NV memory API
--------------------------------------------------------------------------------
 Comments:
================================================================================
*/
...
#if defined(HAL_NV_USE_WORKER_TASK) && (HAL_NV_USE_WORKER_TASK != 0)
			|| !req_queue_len || !sem_pool_len || !bpool
			|| !mdev->dev_requests || !mdev->dev_semaphores
#endif
...
#else
		while (NVOP_IN_PROGRESS == NV_MemDeviceLock(mdev))
#endif
		{
			;
		}
...
static NV_SemaphorePool nv_sem_pool_create(uint32_t no_sems)
{
	...
	if (no_sems)
	{
		retval = HEAP_Alloc(sizeof(NV_SemaphorePool_T));

		if (retval)
		{
			retval->semaphores = HEAP_Alloc(no_sems * sizeof(NV_Semaphore_T));
...
static NV_RequestQueue nv_request_queue_create(uint32_t qlen)
{
	...
	if (qlen)
	{
		retval = HEAP_Alloc(sizeof(NV_RequestQueue_T));
		if (retval)
		{
			retval->requests = HEAP_Alloc(qlen * sizeof(NV_Request_T));
			if (retval->requests)
			{
...
/*! \file hal_nv.c
    \brief HAL non-volatile memory module
