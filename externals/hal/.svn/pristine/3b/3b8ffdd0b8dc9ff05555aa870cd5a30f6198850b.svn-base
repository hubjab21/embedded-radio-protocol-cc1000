...
#define PLL_FREQUENCY 64000000
#define HCLK_FREQUENCY 32000000
...
int main(void){
	...
	// setup PLL
	CC_SetClockSource(cc_stm32, STM32_PLL_CLOCK, STM32_HSE_CLOCK_SOURCE, PLL_FREQUENCY);
	// setup SYSCLK (At the end)
	CC_SetClockSource(cc_stm32, STM32_SYSCLK_CLOCK, STM32_PLL_CLOCK_SOURCE, PLL_FREQUENCY);
	// setup HCLK
	CC_SetClockSource(cc_stm32, STM32_HCLK_CLOCK, STM32_SYSCLK_CLOCK_SOURCE, HCLK_FREQUENCY);
	// setup PCLK1 max 42MHz
	CC_SetClockSource(cc_stm32, STM32_PCLK1_CLOCK, STM32_HCLK_CLOCK_SOURCE, 16000000);
	// setup PCLK1 max 84MHz
	CC_SetClockSource(cc_stm32, STM32_PCLK2_CLOCK, STM32_HCLK_CLOCK_SOURCE, 16000000);
	// setup RTC clock
	CC_SetClockSource(cc_stm32, STM32_RTC_CLOCK, STM32_HSE_CLOCK_SOURCE, 32000);
	...
	CC_ClockOnPin(STM32_PLL_CLOCK, 2, MCO_PIN);
	...
	CC_ClockOnPin(STM32_HSI_CLOCK, 1, MCO_PIN);
	...
	CC_ClockOnPin(STM32_HSE_CLOCK, 1, MCO_PIN);
	...
	CC_ClockOnPin(STM32_LSE_CLOCK, 1, MCO_PIN);
	...
	CC_ClockOnPin(STM32_SYSCLK_CLOCK, 2, MCO2_PIN);

	DIAG0_LogNL(0);
	DIAG0_LogMsg(0, "Change pin to PC9 !!!");
	DIAG0_LogNL(0);
	DIAG0_LogMsg(0, "You should get SYSCLK/2 frequency now: ~");
	DIAG0_LogUINT(0,CC_GetClockFreq(cc_stm32, STM32_SYSCLK_CLOCK), 10);
	DIAG0_LogMsg(0, " Hz");
	DIAG0_LogNL(0);
	DIAG0_LogNL(0);
	DIAG0_LogMsg(0, "//////// To go to end of test push any button ////////// ");
	DIAG0_LogNL(0);
	DIAG0_LogNL(0);
