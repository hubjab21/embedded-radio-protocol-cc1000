...
--------------------------------------------------------------------------------
 File: 			hal_heap2.c
 Author:        Artur Lipowski, Łukasz Krzak
 Purpose:       HALFRED
 Compiler:      %COMPILER%
...
#error Possible values for HAL_HEAP_ALIGNMENT: 0, 1, 2, 4, 8
    ...
    blocks_required = ((BlockSizeAddOp_T) size + (BlockSizeAddOp_T) sizeof(BlockSize_T) + (BlockSizeAddOp_T) sizeof(Block_T) - 1) / (BlockSizeAddOp_T) BLOCK_SIZE;
    ...
    //
    for (block = sentinel->header.ptrs.next;
         block != sentinel;
         block = block->header.ptrs.next)
    {
        merge_blocks(block);
